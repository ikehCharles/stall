...
const jsonResponse = (status: number, body: Record<string, any>) =>
      ...
      try {
        ...
        const payload = {
          ...
          variants: [{
            ...
            sku: booking.id,
            name: booking.invoice_number,
            price: { amount: Math.round(booking.total_amount * 100), currencyId: 'GBP' },
            costPrice: { amount: Math.round(booking.total_amount * 100), currencyId: 'GBP' },
            vatPercentage: 0,
          }],
        ...
        if (!resp.ok) {
          const err = await resp.text()
          console.error(`Failed for ${booking.invoice_number}:`, resp.status, err)
          failCount++
          continue
        }
        ...
        console.log(`Zettle created product for booking ${booking.id}:`, result)
        ...
        if (updateErr) {
          ...
          successCount++
          console.log(`Synced: ${booking.invoice_number} → Zettle UUID ${result.uuid}`)
        }
