        ...
        <CardContent className="p-8">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Marketplace Management Platform</p>
            </div>
            <div className="text-right">
              <h2 className="text-2xl font-bold text-foreground">INVOICE</h2>
              <p className="text-muted-foreground">#{booking.invoice_number}</p>
              <p className="text-muted-foreground">Date: {format(new Date(booking.created_at), 'PPP')}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <div className="space-y-1">
                <p className="font-medium">{booking.profiles?.full_name || 'Vendor'}</p>
                <p className="text-muted-foreground">{booking.profiles?.email}</p>
                {booking.profiles?.phone_number && (
                  <p className="text-muted-foreground">{booking.profiles.phone_number}</p>
                )}
                {booking.profiles?.company_name && (
                  <p className="text-muted-foreground">{booking.profiles.company_name}</p>
                )}
                {booking.profiles?.address && (
                  <p className="text-muted-foreground">{booking.profiles.address}</p>
                )}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-1">
                <p className="font-medium">{booking.markets?.name || 'Market Event'}</p>
                <p className="text-muted-foreground">
                  Start Date: {booking.markets?.start_at ? format(new Date(booking.markets.start_at), 'PPP') : 'N/A'}
                </p>
                <p className="text-muted-foreground">
                  End Date: {booking.markets?.end_at ? format(new Date(booking.markets.end_at), 'PPP') : 'N/A'}
                </p>
                <p className="text-muted-foreground">
              ...
              <table className="w-full">
                <thead className="bg-muted">
                  <tr>
                    ...
                    <th className="text-left py-3 px-4 font-medium">Selected Dates</th>
                    <th className="text-right py-3 px-4 font-medium">Days</th>
                    <th className="text-right py-3 px-4 font-medium">Price/Day</th>
                    <th className="text-right py-3 px-4 font-medium">Subtotal</th>
                  </tr>
                </thead>
                <tbody>
                  {booking.booking_stalls?.map((bs) => {
                    const stallDates = booking.booking_dates?.filter(
                    ...
                    return (
                      <tr key={bs.id} className="border-t">
                        ...
                        <td className="py-3 px-4 text-muted-foreground">
                          {stallDates.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {stallDates.map((bd) => (
                                <span key={bd.id} className="text-xs bg-muted px-2 py-1 rounded">
                        ...
                        <td className="py-3 px-4 text-right">${pricePerDay.toFixed(2)}</td>
          ...
          <div className="mt-6 flex justify-end">
            <div className={`px-4 py-2 rounded-full text-sm font-medium ${
              ...
              Payment Status: {booking.payment_status?.toUpperCase() || 'PENDING'}
            </div>
          </div>
